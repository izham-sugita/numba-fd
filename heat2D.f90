...
!  !$omp end parallel
  ...
  do iter = 0, itermax, istep
     ...
     do inner = 0, istep
        ...
        do j = 2, jmax-1
           do i = 2, imax-1
              ...
              !rhs(i,j) = T(i,j) + alpha*( T(i-1,j) - 2.0*T(i,j) + T(i+1,j) ) &
              !     & +  beta*( T(i,j-1) - 2.0*T(i,j) + T(i,j+1) )

              Tn(i,j) = T(i,j) + alpha*( T(i-1,j) - 2.0*T(i,j) + T(i+1,j) ) &
                   & +  beta*( T(i,j-1) - 2.0*T(i,j) + T(i,j+1) )
           end do
        end do
        !$omp end parallel do
     ...
     write(*,100) iter*dt, Told(icenter,jcenter), T(icenter,jcenter), Tn(icenter,jcenter)
     100 format(4E15.8)
  ...
  print *, tstart
  print *, tend
  print *, "Elapsed time ", tend-tstart, " secs"
